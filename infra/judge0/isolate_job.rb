class IsolateJob < ApplicationJob
  ...
  def extract_archive
    ...
    #{submission.enable_per_process_and_thread_memory_limit ? "" : "--cg-mem=#{Config::MAX_MEMORY_LIMIT}"} \
  ...
  def run
    ...
    #{submission.enable_per_process_and_thread_time_limit ? (cgroups.present? ? "--no-cg-timing" : "") : "--cg-timing"} \
    #{submission.enable_per_process_and_thread_memory_limit ? "" : "--cg-mem=#{submission.memory_limit}"} \
